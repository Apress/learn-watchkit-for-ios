...
class ConfigurationController: WKInterfaceController {
    ...
    override func awakeWithContext(context: AnyObject?) {
        ...
        // Add menu items
        addMenuItemWithItemIcon(.Repeat, title: "Reset", action: "onResetClicked")
        addMenuItemWithImageNamed("RandomMenuIcon", title: "Random", action: "onRandomClicked")
    }
    ...
    func onResetClicked() -> Void {
        let color = controllerContext!.textAttributes.color
        let font = controllerContext!.textAttributes.font
        if color != selectedColor || font != selectedFont {
            selectedColor = color
            selectedFont = font
            updateCheckMarks()
        }
    ...
    func onRandomClicked() -> Void {
        let color = TextAttributes.colors[Int(arc4random_uniform(UInt32(TextAttributes.colors.count)))]
        let font = TextAttributes.fonts[Int(arc4random_uniform(UInt32(TextAttributes.fonts.count)))]
        if color != selectedColor || font != selectedFont {
            selectedColor = color
            selectedFont = font
            updateCheckMarks()
